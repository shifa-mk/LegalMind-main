...
return (
  <div className="bg-blue-900 text-white shadow-sm sticky top-0 z-50">
    <div className="flex items-center justify-between max-w-7xl mx-auto h-16 px-4">
      ...
      {/* 2. RIGHT SIDE: Auth, Mobile Menu, & Profile */}
      <div className="flex items-center gap-2 md:gap-4">
        {!token ? (
          /* Login/Signup (No Token) */
          <div className="flex items-center gap-3">
            <Link to="/">
              <Button variant="outline" className="text-white border-white hover:bg-white hover:text-blue-900 h-9 px-3 text-sm">
                Login
              </Button>
            </Link>
            <Link to="/signup">
              <Button className="bg-red-600 hover:bg-red-700 text-white border-none h-9 px-3 text-sm">
                Signup
              </Button>
            </Link>
          </div>
        ) : (
          /* Authenticated Tools (Token Exists) */
          <div className="flex items-center gap-2 md:gap-4">
            
            {/* MOBILE MENU DROPDOWN: Hidden on Desktop (md), visible on Mobile */}
